...
// ─── WIFI / AGENTE ────────────────────────────────────
#define WIFI_SSID   "iPhone de Juan camilo"
#define WIFI_PASS   "contrasea"
#define AGENT_IP    "172.20.10.10"   // IP de tu PC con el agente
...
void setmode_callback(const void * req, void * res) {
  ...
  s->success = true;
  s->message.data = modoAutonomo ? (char*)"Modo AUTONOMO" : (char*)"Modo REMOTO";
  s->message.size = modoAutonomo ? 13 : 12;
  s->message.capacity = modoAutonomo ? 14 : 13;
}
...
void setup() {
  ...
  RCCHECK(rclc_timer_init_default(&pub_timer, &support,
    RCL_MS_TO_NS(PUB_PERIOD_MS), pub_timer_callback));
